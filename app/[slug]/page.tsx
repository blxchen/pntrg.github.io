...
function PeoplePage() {
  ...
  return <><PageHero index="03 / Team" eyebrow="The lab" title="Small team. Wide field of view." intro="PNTL was founded by Brandon Chen and Naveed Ahmed to connect rigorous navigation research with the systems and environments that need it." />
    <section className="team-directory"><div className="section-heading" data-reveal><p>Founding team</p><h2>Different lenses.<br />One dependable system.</h2></div><div className="team-grid">{people.map((person, index) => <Link className={`team-member-card ${index === 1 ? 'team-member-alt' : ''}`} data-reveal href={`/research-profile?person=${person.id}`} key={person.id}><div className="team-monogram"><span>{person.initials}</span><i /></div><div className="team-card-copy"><p>{person.role}</p><h3>{person.name}</h3><div>{person.text}</div><ul>{person.interests.map((interest) => <li key={interest}>{interest}</li>)}</ul><b>Open research profile <span>↗</span></b></div><small>0{index + 1}</small></Link>)}</div></section>
